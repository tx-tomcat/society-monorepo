...
export const CheckboxIcon = ({ checked = false }: IconProps) => {
  const color = checked ? colors.rose[400] : colors.charcoal[400];
  return (
    <MotiView
      style={{
        height: SIZE,
...
export const RadioIcon = ({ checked = false }: IconProps) => {
  ...
  return (
    ...
    >
      <MotiView
        className={`size-[10px] rounded-[10px] ${checked && 'bg-rose-400'} `}
        from={{ opacity: 0 }}
        animate={{ opacity: checked ? 1 : 0 }}
...
export const SwitchIcon = ({ checked = false }: IconProps) => {
  ...
  const backgroundColor = checked ? colors.rose[400] : colors.charcoal[400];
