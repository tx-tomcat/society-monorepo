...
export default function CompanionPhoneVerification() {
  const router = useRouter();
  const { t } = useTranslation();

  const setAuthData = useCompanionOnboarding.use.setAuthData();
  ...
  React.useEffect(() => {
    if (isSuccess) {
      setAuthData({ phone: phoneNumber, authMethod: 'phone' });
  ...
  }, [isSuccess, phoneNumber, setAuthData, markStepComplete, router]);
  ...
  return (
    <View className="flex-1 bg-warmwhite">
      ...
      <CompanionHeader
        title={t('onboarding.phone.header')}
        onBack={handleBack}
        rightElement={
          <Text className="text-sm text-text-tertiary">
            {`${t('companion.onboard.step')} 2/4`}
          </Text>
        }
      />
          ...
          {isSuccess ? (
            ...
            <>
              <MotiView
                ...
                className="mb-8"
              >
                <View className="mb-4 size-16 items-center justify-center rounded-full bg-lavender-900/10">
                  <Phone color={colors.lavender[400]} width={28} height={28} />
                </View>
