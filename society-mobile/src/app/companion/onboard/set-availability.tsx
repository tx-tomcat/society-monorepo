...
const DAYS: DayOfWeek[] = [
  { id: 'mon', shortKey: 'common.days.mon_short', fullKey: 'common.days.monday' },
  { id: 'tue', shortKey: 'common.days.tue_short', fullKey: 'common.days.tuesday' },
  { id: 'wed', shortKey: 'common.days.wed_short', fullKey: 'common.days.wednesday' },
  { id: 'thu', shortKey: 'common.days.thu_short', fullKey: 'common.days.thursday' },
  { id: 'fri', shortKey: 'common.days.fri_short', fullKey: 'common.days.friday' },
  { id: 'sat', shortKey: 'common.days.sat_short', fullKey: 'common.days.saturday' },
  { id: 'sun', shortKey: 'common.days.sun_short', fullKey: 'common.days.sunday' },
];

const TIME_SLOTS = [
  { id: 'morning', labelKey: 'companion.onboard.set_availability.slots.morning', time: '8:00 - 12:00' },
  { id: 'afternoon', labelKey: 'companion.onboard.set_availability.slots.afternoon', time: '12:00 - 17:00' },
  { id: 'evening', labelKey: 'companion.onboard.set_availability.slots.evening', time: '17:00 - 22:00' },
];

export default function SetAvailability() {
  const router = useRouter();
  const { t } = useTranslation();
  const [selectedDays, setSelectedDays] = React.useState<string[]>(['sat', 'sun']);
  const [selectedSlots, setSelectedSlots] = React.useState<string[]>(['afternoon', 'evening']);
  const [advanceNotice, setAdvanceNotice] = React.useState('24');

  const handleBack = React.useCallback(() => {
    router.back();
  }, [router]);

  const handleToggleDay = React.useCallback((dayId: string) => {
  ...
  const handleToggleSlot = React.useCallback((slotId: string) => {
    ...
    <View className="flex-1 bg-warmwhite">
      ...
      <SafeAreaView edges={['top']}>
        <View className="flex-row items-center gap-4 border-b border-border-light px-4 py-3">
          ...
          <Text className="text-sm text-text-tertiary">{t('companion.onboard.step', { current: 4, total: 4 })}</Text>
        </View>
      </SafeAreaView>
      ...
      >
        ...
        >
          ...
          <Text className="mb-4 text-sm text-text-secondary">
            {t('companion.onboard.set_availability.preferred_slots_description')}
          </Text>
          ...
          <View className="gap-3">
            {TIME_SLOTS.map((slot) => {
              ...
              return (
                ...
                >
                  <View>
                    <Text className="font-semibold text-midnight">{t(slot.labelKey)}</Text>
                    <Text className="text-sm text-text-secondary">{slot.time}</Text>
                  </View>
                  <View
